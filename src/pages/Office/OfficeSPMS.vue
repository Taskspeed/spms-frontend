  ...
  // UWP Validation Dialog refs
  const uwpValidationDialog = ref(false),
    uwpValidationMessage = ref(''),
    uwpIncompleteItems = ref([])
  ...
  /**
   * Get ONLY direct employees of the selected node (not from child nodes)
   * This prevents showing employees from sub-divisions when viewing a parent level
   */
  const employees = computed(() => {
    ...
    // For organizational nodes, ONLY get direct employee children (not from nested structures)
    // Filter the immediate children to get only employees
  ...
  /**
   * Get completion ratio for a node from SPMS store
   */
  ...
  const getCompletionColor = (nodeId) => {
    ...
    if (completion.isCompleted) {
      return 'positive' // Green - completed
    }
    if (completion.total === 0) {
      return 'grey-7' // No UWP required
    }
    return 'warning' // Yellow - incomplete
  }
  ...
  /**
   * Create Unit Work Plan with validation
   */
  const createUnitWorkPlan = () => {
    ...
    // All validation passed, proceed to create UWP
